...
ass_f1 <- bf(IQR_z ~ Diagnosis + (1|Language:ID))
...
ass_f1_prior <- c(
  ...
  prior(normal(0, 0.3), class = sd, coef = Intercept, group = Language:ID),
  prior(exponential(1), class = sigma)
)
...
View(posterior_gauss)

ggplot(posterior_gauss) + #plotting the intercept prior + posterior
  theme_classic() +
  geom_density(aes(prior_Intercept), fill="red", alpha=0.3) +
  geom_density(aes(Intercept), fill="blue", alpha=0.5)
...
####STUDENT T####
#################
ass_f1_student <- bf(IQR_z ~ Diagnosis + (1|Language) + (1|ID))
