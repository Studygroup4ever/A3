...
```{r}
# Renaming posterior samples for better overview 
posterior_student <- posterior_stud # Posterior sample from model from step 3
posterior_student_MA <- samples_stud # Posterior sample from model from step 4 w. meta analytic prior

# Renaming posterior models
ass_m1_student_post <- ass_m2_post
ass_MA_student_post_m1 <- m2_mp

# Finding the difference between experimental groups
posterior_student$beta_diff <- posterior_student$b_DiagnosisTD - posterior_student$b_DiagnosisASD

posteriors_all <-
  data.frame(
    "values" = c(
      posterior_student$beta_diff,
      posterior_student_MA$b_DiagnosisASD
    ),
    'Estimate' = c(rep(
      "step 3", length(posterior_student$beta_diff)
    ), rep(
      "step 4", length(posterior_student_MA$b_DiagnosisASD)
    ))
  )
...
ggplot(posteriors_all, aes(values, fill = Estimate)) +
  ...
  ggtitle("Posteriors for beta step 3 and 4")
