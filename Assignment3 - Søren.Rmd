...
```{r}
# Load the data
d <- read.delim('Ass3_MetaAnalysisData.tsv')[1:41,]
...
```{r}
# Changing data structure - we need it to be long 
d_long <- gather(d, condition, measurement, control:cond2, factor_key=TRUE)
...
?rma
ma_out <- rma(data = d, yi = yi, sei = sei, slab = dat$study)
summary(ma_out)
```
