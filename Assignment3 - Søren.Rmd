...
p1 <- c(
  ...
  prior(exp(1), class = sd, group = Paper),
  prior(normal(0.5, 0.25), class = sd, coef = Intercept, group = Paper) # mean at -0.5 because that's around the SME, sd set to 0.5 after looking at the summary(d$Effectsize) and looking at minimum / maximum / 1st / 3rd quantile.
  )

p1_stud <- c(
  prior(student_t(3, -0.5, 0.5), class = Intercept), # Prior for intercept
  prior(exp(1), class = sd, group = Paper), # Prior for sd between papers
  prior(normal(0.5, 0.25), class = sd, coef = Intercept, group = Paper) # Prior for sd for intercept between papers
  )
...
pp_priornorm + pp_postnorm + pp_priorstud + pp_poststud
```
...
posterior_m0 <- posterior_samples(m0) 

# Creating a long format of the posterior to plot the different distributions together
posterior_m0 %>% pivot_longer(c(Intercept, prior_Intercept), names_to = "Estimates") %>%
...
# Creating a long format of the posterior to plot the different distributions together
  pivot_longer(c(sd_Paper__Intercept, prior_sd_Paper__Intercept), names_to = "Estimates") %>%
