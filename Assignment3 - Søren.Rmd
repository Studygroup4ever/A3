...
long[complete.cases(long['Mean']),] %>% 
  ggplot(aes(x = Paper)) +
  geom_point(aes(y = Mean), shape = 1) +
...
# Formatting data
d[complete.cases(d['PitchMeanTD_Mean']),] %>% 
  mutate(n = Paper %>% as.factor() %>% as.numeric(),
         colorTD = 'TD',
         colorASD = 'ASD') %>% 
...
# Building plot
  ...
  geom_linerange(aes(x = n + 0.3,
                     ...
                     color = colorASD)) +
  geom_vline(xintercept = c(1:15 + 0.65), alpha = 0.3) +
  geom_hline(yintercept = (mean(d[complete.cases(d['PitchMeanTD_Mean']),]['PitchMeanTD_Mean'][,1]) +
                           mean(d[complete.cases(d['PitchMeanASD_Mean']),]['PitchMeanASD_Mean'][,1]))/2, lty = 2) +
  scale_x_discrete(limits = factor(1:16), 
                   name = 'Study',
                   labels = unique(d[complete.cases(d['PitchMeanASD_Mean']),]['Paper'])[,1]) + 
  theme_classic() + 
  ylab('Mean Pitch Variability') +
