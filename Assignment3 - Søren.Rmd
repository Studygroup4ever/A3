...
# model priors
p1 <- c(
  prior(normal(-0.5, 0.5), class = Intercept), # mean at -0.5 because that's around the SME, sd set to 0.5 after looking at the summary(d$Effectsize) and looking at minimum / maximum / 1st / 3rd quantile.
...
inter_plot <- function(df){
  df %>% pivot_longer(c(Intercept, prior_Intercept), names_to = "Estimates") %>%
...
inter_plot(posterior_m1)
sd_plot(posterior_m1)

inter_plot(posterior_m1_stud)
sd_plot(posterior_m1_stud)
