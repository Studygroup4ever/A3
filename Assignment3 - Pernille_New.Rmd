...
d[complete.cases(d['EffectSize']),] %>% 
  ...
  geom_point() +
  scale_y_reverse() 
  scale_y_continuous(limits = c(0, max(d$StandardError))) 

funnelplotdata <- funnel(ma_out)

# Create vector of colors
my_colors <- c('red','blue')[(d$Paper)]

length(unique(d$Paper))
# Plot over points
...
```{r}
# the two models
f0 <- bf(EffectSize | se(StandardError) ~ 0 + (1 | Paper))
...
# get priors
get_prior(f0, data = d_clean)
get_prior(f1, data = d_clean)

# null model priors - trying with gaussians / students t
p0 <- c(
  #prior(normal(0, 1), class = sd),
  #prior(normal(1, 0.5), class = sd, group = Paper),
  prior(normal(-0.5, 0.5), class = sd, coef = Intercept, group = Paper) # mean at -0.5 because that's around the SME, sd set to 0.5 after looking at the summary(d$Effectsize) and looking at minimum / maximum / 1st / 3rd quantile.
  )
